...
class ThreadedPromises {
  public Thread thread;
  public Collection<Promise> promises = Collections.synchronizedCollection(new ArrayList<Promise>());

  ThreadedPromises(Thread thread, Promise promise) {
    this.thread = thread;
  ...
  public void resolveAll(ArrayList<HashMap<String, String>> result) {
    if (this.promises.size() > 0) {
      this.promises.forEach((promise) -> {
        WritableArray ports = new WritableNativeArray();

        for (HashMap<String, String> port : result) {
          WritableMap portInfo = new WritableNativeMap();

          for (String property : port.keySet()) {
            portInfo.putString(property, port.get(property));
          }
  ...
  public void rejectAll(String message, Exception exception) {
    if (this.promises.size() > 0) {
      this.promises.forEach((promise) -> promise.reject(message, exception));
      this.promises.clear();
    }
  }
}
...
public class RNStarPrntModule extends ReactContextBaseJavaModule {
  ...
  private Thread threadedPortDiscovery(final String strInterface) {
    return new Thread(() -> {
      ThreadedPromises currentThreadedPromises = this.portDiscoveryThreadPool.get(strInterface);
      CallWithPermission callPermissionWrapper = new CallWithPermission(
        (AppCompatActivity) this.reactContext.getCurrentActivity(),
      ...
      try {
        switch (strInterface) {
          case "LAN":
            currentThreadedPromises.resolveAll(getPortDiscovery("LAN"));
            break;
          case "Bluetooth":
            currentThreadedPromises.resolveAll(callPermissionWrapper.callMayThrow(
              () -> getPortDiscovery("Bluetooth"),
              () -> currentThreadedPromises.rejectAll("BLUETOOH_PERMISSION_DENIED", null)
            ));
            break;
          case "USB":
            currentThreadedPromises.resolveAll(getPortDiscovery("USB"));
            break;
          default:
            currentThreadedPromises.resolveAll(getPortDiscovery("All"));
            break;
        }
        this.portDiscoveryThreadPool.remove(strInterface);
  ...
  public void portDiscovery(final String strInterface, final Promise promise) {
    ...
    if (currentThreadedPromises == null) {
      Thread newThread = this.threadedPortDiscovery(strInterface);
      this.portDiscoveryThreadPool.put(strInterface, new ThreadedPromises(newThread, promise));
      newThread.start();
    } else {
  ...
  private ArrayList<HashMap<String, String>> getPortDiscovery(String interfaceName) throws StarIOPortException {
    ...
    for (PortInfo discovery : arrayDiscovery) {
      HashMap<String, String> portData = new HashMap<>();
      if (discovery.getPortName().startsWith("BT:")) {
        portData.put("portName", "BT:" + discovery.getMacAddress());
      } else {
        portData.put("portName", discovery.getPortName());
      }

      if (!discovery.getMacAddress().equals("")) {
        portData.put("macAddress", discovery.getMacAddress());
        if (discovery.getPortName().startsWith("BT:")) {
          portData.put("modelName", discovery.getPortName());
        } else if (!discovery.getModelName().equals("")) {
          portData.put("modelName", discovery.getModelName());
        }
        ...
        if (!discovery.getUSBSerialNumber().equals(" SN:")) {
          portData.put("USBSerialNumber", discovery.getUSBSerialNumber());
        }
      }
    ...
    for (int i = 0; i < printCommands.size(); i++) {
      ...
      if (command.hasKey("appendCharacterSpace"))
        ...
        if (command.hasKey("data"))
          builder.appendAbsolutePosition((command.getString("data").getBytes(encoding)), command.getInt("appendAbsolutePosition"));
        else builder.appendAbsolutePosition(command.getInt("appendAbsolutePosition"));
      } else if (command.hasKey("appendAlignment")) {
        if (command.hasKey("data"))
          builder.appendAlignment((command.getString("data").getBytes(encoding)), getAlignment(command.getString("appendAlignment")));
        ...
        if (command.hasKey("absolutePosition")) {
          ...
          builder.appendBarcode(command.getString("appendBarcode").getBytes(encoding), barcodeSymbology, barcodeWidth, height, hri);
        ...
        if (command.hasKey("absolutePosition")) {
          ...
          builder.appendQrCode(command.getString("appendQrCode").getBytes(encoding), qrCodeModel, qrCodeLevel, cell);
