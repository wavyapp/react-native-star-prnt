...
class ReactNativeStarPrinter: RCTEventEmitter {
  private var manager: StarDeviceDiscoveryManager? = nil
  var RNStarPrnt_hasListeners: bool;
  ...
  @objc()
  func startObserving() {
    self.RNStarPrnt_hasListeners = YES
  ...
  @objc()
  func stopObserving() {
    self.RNStarPrnt_hasListeners = NO
  ...
  func searchPrinter(
    ...
    do {
      try manager = StarDeviceDiscoveryManagerFactory.create(interfaceTypes: [InterfaceType.bluetooth, InterfaceType.bluetoothLE])
      ...
      if errorCode == StarIO10ErrorCode.bluetoothUnavailable {
        ...
        rejecter("PRINTER_SEARCH_ERROR_NO_BLUETOOH", message);
      }
    } catch let error {
      rejecter("PRINTER_SEARCH_ERROR", "Error while searching for printer", error);
    }
  }
  
//  @objc()
//  func portDiscovery(portType: String, _ resolve: RCTPromiseResolveBlock, rejecter reject: RCTPromiseRejectBlock)
//  {
//    var info: [PortInfo] = []
//    
//    do {
//      if (portType == "All" || portType == "Bluetooth") {
//        info = try SMPort.searchPrinter(target: "BT:")
//      }
//      
//      if ([portType isEqualToString:@"All"] || [portType isEqualToString:@"LAN"]) {
//        info = try SMPort.searchPrinter(target: "TCP:")
//      }
//      
//      if ([portType isEqualToString:@"All"] || [portType isEqualToString:@"BluetoothLE"]) {
//        info = try SMPort.searchPrinter(target: "BLE:")
//      }
//      
//      if ([portType isEqualToString:@"All"] || [portType isEqualToString:@"USB"]) {
//        info = try SMPort.searchPrinter(target: "USB:")
//      }
//    } catch {
//      rejecter("PRINTER_SEARCH_ERROR", "Error while searching for printer", error);
//    }
...
//      resolve(info);
//    } else {
//      rejecter("NO_PRINTERS_FOUND", "Printers not found");
//    }
//  }
...
//     [builder appendInternational:SDCBInternationalTypeFrance];
    
//     [builder appendCodePage     :SDCBCodePageTypeCP1252];
    
//     for (id command in displayCommands) {
//       NSString *string = [command valueForKey:@"appendCustomerDisplay"];
//       const char *array = [ string UTF8String ];
//       [builder appendBytes:array length:[string length]];
//     }
...
//   RCT_REMAP_METHOD(appendTurnOn, portName:(NSString *)portName
//                    emulation:(NSString *)emulation
//                    printCommands:(NSArray *) printCommands
//                    appendTurnOnResolver:(RCTPromiseResolveBlock)resolve
//                    rejecter:(RCTPromiseRejectBlock)reject)
//   {
//     NSString *portSettings = [self getPortSettingsOption:emulation];
